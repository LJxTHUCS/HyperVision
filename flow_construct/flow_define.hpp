...
namespace Hypervision
{
    ...
    public:
        ...
        basic_flow(const decltype(str) str, const decltype(end) end, const decltype(code) code,
                   const decltype(p_packet_p_seq) p_packet_p_seq, const decltype(p_reverse_index) p_reverse_index) : str(str), end(end), code(code), p_packet_p_seq(p_packet_p_seq), p_reverse_index(p_reverse_index) {}

        basic_flow(const decltype(p_packet_p_seq) p_packet_p_seq, const decltype(p_reverse_index) p_reverse_index) : p_packet_p_seq(p_packet_p_seq), p_reverse_index(p_reverse_index)
        {
    ...
    public:
        ...
        tuple5_flow4(const tuple5_conn4 &flow_id,
                     ...
                     const decltype(p_reverse_index) p_reverse_index) : flow_id(flow_id), basic_flow(str, end, code, p_packet_p_seq, p_reverse_index) {}

        tuple5_flow4(const tuple5_conn4 &flow_id, const decltype(p_packet_p_seq) p_packet_p_seq,
                     const decltype(p_reverse_index) p_reverse_index) : flow_id(flow_id), basic_flow(p_packet_p_seq, p_reverse_index) {}
