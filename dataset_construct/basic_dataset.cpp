...
void basic_dataset::do_dataset_construct(size_t multiplex)
{
    ...
    p_parse_train = make_shared<decltype(p_parse_train)::element_type>(p_parse_result->begin(), p_parse_result->begin() + line);
    p_parse_test = make_shared<decltype(p_parse_test)::element_type>(p_parse_result->begin() + line, p_parse_result->end());
