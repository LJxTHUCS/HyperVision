...
namespace Hypervision
{
    ...
    struct basic_packet
    {
        ...
        explicit basic_packet(const decltype(ts) ts, const decltype(tp) tp, const decltype(len) len) : ts(ts), tp(tp), len(len) {}
    ...
    struct basic_packet4 final : public basic_packet
    {
        ...
        explicit basic_packet4(const pkt_addr4_t s_IP,
                               ...
                               const decltype(ts) ts, const decltype(tp) tp, const decltype(len) len) : flow_id(s_IP, d_IP, s_port, d_port), basic_packet(ts, tp, len) {}
        explicit basic_packet4(const string s_IP,
                               const string d_IP,
                               const pkt_port_t s_port,
                               const pkt_port_t d_port,
                               const decltype(ts) ts, const decltype(tp) tp, const decltype(len) len) : flow_id(convert_str_addr4(s_IP), convert_str_addr4(d_IP), s_port, d_port), basic_packet(ts, tp, len) {}
        explicit basic_packet4(const decltype(flow_id) flow_id,
                               const decltype(ts) ts, const decltype(tp) tp, const decltype(len) len) : flow_id(flow_id), basic_packet(ts, tp, len) {}
    ...
    struct basic_packet6 final : public basic_packet
    {
        ...
        explicit basic_packet6(const string s_IP,
                               ...
                               const pkt_port_t s_port,
                               const pkt_port_t d_port,
                               const decltype(ts) ts, const decltype(tp) tp, const decltype(len) len) : flow_id(convert_str_addr6(s_IP), convert_str_addr6(d_IP), s_port, d_port), basic_packet(ts, tp, len) {}
