...
auto traffic_graph::connected_component() const -> shared_ptr<component>
{
    ...
    for (const auto &ref : long_edge_out)
    {
        ...
        {
            _edge_raw.push_back({_vertex[ref.first], _vertex[p_long_edge->at(idx)->get_dst_str()]});
        }
    }
    for (const auto &ref : short_edge_out)
    {
        for (const auto idx : ref.second)
        {
            _edge_raw.push_back({_vertex[ref.first], _vertex[p_short_edge->at(idx)->get_dst_str()]});
        }
    }
    ...
    for (const auto &item : parts)
    {
        ...
        const auto get_out_degree_addr = [&](size_t _idx) -> size_t
        {
            ...
            return long_edge_out.count(_addr) ? long_edge_out.at(_addr).size() : 0 + short_edge_out.count(_addr) ? short_edge_out.at(_addr).size()
                                                                                                                 : 0;
        };
        const auto get_in_degree_addr = [&](size_t _idx) -> size_t
        {
            const auto &_addr = _vertex_reverse[_idx];
            return long_edge_in.count(_addr) ? long_edge_in.at(_addr).size() : 0 + short_edge_in.count(_addr) ? short_edge_in.at(_addr).size()
                                                                                                              : 0;
        };
...
auto traffic_graph::component_select(const shared_ptr<component> p_com) const -> shared_ptr<vector<size_t>>
{
    ...
    sort(loss_vec.begin(), loss_vec.end(), [&](decltype(loss_vec)::value_type &a, decltype(loss_vec)::value_type &b) -> bool
